...
% %         k = reshape(k,3,1);
% %         l = a * k;
%         img_inv(i,j,1) = (img(i,j,1) - img(i,j,2))/sqrt(2);
%         img_inv(i,j,2) = (img(i,j,1) + img(i,j,2) - 2*img(i,j,3))/sqrt(6);
%         img_inv(i,j,3) = (img(i,j,1) + img(i,j,2) + img(i,j,3))/sqrt(3);
%         %img_inv(i,j,1) = img_inv(i,j,1)/img_inv(i,j,3);
%     end
% end
% %imshow(img);
% %corners = detectHarrisFeatures(img_inv(:,:,1));
% [f,d] = vl_sift(img_inv(:,:,2));
% [f1,d1] = vl_sift(img_inv(:,:,1));
% imshow(I); hold on
