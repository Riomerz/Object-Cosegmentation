...
def load_data(mode):
	...
	#img1 = cv2.resize(cv2.imread(os.getcwd() + txt[i][0][7:]),(360,480))
        img1 = cv2.imread(os.getcwd() + txt[i][0][7:])
        data.append(normalized(img1))

	#img2 = cv2.resize(cv2.imread(os.getcwd() + txt[i][1][7:][:-1]), (360,480))
	img2 = cv2.imread(os.getcwd() + txt[i][1][7:][:-1])
        label.append(one_hot_it(img2[:,:,0]))
        print('.',end='')
